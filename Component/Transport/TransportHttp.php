...
{   
	...
	public function sendMessage($to, $message, $from = "", $callback = true)
	{
		...
		if (isset($result['ERR']))
		{
			return $this->wrapResponse(Transport::RESULT_FAILURE, (string) $result['ERR']);
		}
		else
		{
			$packet = array();
    ...
    {
    	$response = $this->transfer()->execute(self::ENDPOINT_GET_BALANCE, $this->buildPost($this->request()));
